<script>
	...
	async function handleSubmit() {
		await submitCallback(selectedPartForComment.comments || null);
	}
</script>
			...
			>
				...
				<div class="flex items-start justify-between p-5 rounded-t">
					<h3 class="text-3xl text-gray-600 font-semibold">
						{selectedPartForComment?.boms_part?.part?.number}
					</h3>
					<button
				...
				<div class="mx-4">
					<p class="mb-1 text-sm">Comments:</p>
					<TextInput bind:value={selectedPartForComment.comments} fullWidth />
				</div>
