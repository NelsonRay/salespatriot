<script>
	...
	async function handleSubmit() {
		await submitCallback(selectedPart.vendor_instructions || null);
	}
</script>
		...
		<div class="relative w-1/3 my-6 mx-auto max-w-3xl">
			...
			>
				...
				<div class="flex items-start justify-between p-5 rounded-t">
					<h3 class="text-3xl text-gray-600 font-semibold">{selectedPart?.number}</h3>
					<button
				...
				<div class="mx-4">
					<p class="mb-1 text-sm">Vendor Instructions:</p>
					<TextInput bind:value={selectedPart.vendor_instructions} fullWidth />
				</div>
