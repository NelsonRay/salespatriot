...
export async function POST({ request, cookies }) {
	...
	const { data: notification, error } = await supabase
		.from('forms_notifications')
		.select(
			'*, form(*, form(name, type), solicitation_matched(solicitation!inner(id)), part(number), rfq(received_at, customer(name)), rfq_public(*))'
		)
		.eq('id', id)
		.limit(1)
		.single();
	...
	if (!notification.form.commercial) {
		userId = notification.form.assignee;
		btnText = `Open ${notification.form.form.name}`;
		subject = `${notification.form.form.name}: ${notification.form.solicitation_matched.solicitation.id}`;
		formLink = `https://salespatriot.com/solicitation-form/${notification.form.id}`;
	} else {
		// if commercial
		...
		userId = notification.form.assignee;
		btnText = `Open ${notification.form.form.name}`;
		if (notification.form.email) {
			subject = 'Confirm Form: New Email';
		} else {
			subject = `${notification.form.form.name}: ${notification.form.part?.number ?? (!notification.form.rfq_public ? notification.form.rfq.customer.name + ' / ' + notification.form.rfq.received_at : notification.form.rfq_public.values.customer.name + ' / ' + notification.form.rfq_public.values.received_at)}`;
		}
		formLink = `https://salespatriot.com/commercial-form/${notification.form.id}`;
	}
